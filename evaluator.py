...
Main evaluation module using Arize Phoenix and LangChain
Phoenix data stored in local .phoenix/ directory - No permission issues!
"""
...
class PhoenixRAGTracer:
    """
    Arize Phoenix integration for RAG tracing
    Stores data in local .phoenix/ directory
    """
    
    def __init__(self, working_dir: str = ".phoenix"):
        """
        Initialize Phoenix session with local storage
        
        Args:
            working_dir: Directory to store Phoenix data (default: .phoenix in current dir)
        """
        # Create phoenix directory in the repo
        phoenix_path = Path(working_dir).absolute()
        phoenix_path.mkdir(exist_ok=True, parents=True)
        
        # Set Phoenix working directory to our local folder
        os.environ["PHOENIX_WORKING_DIR"] = str(phoenix_path)
        
        logger.info(f"📁 Phoenix data will be stored in: {phoenix_path}")
        
        # Launch Phoenix with local storage
        self.session = px.launch_app()
        logger.info(f"🌍 Phoenix UI available at: {self.session.url}")
        logger.info(f"💾 Database location: {phoenix_path / 'phoenix.db'}")
    
    def log_retrieval_results(self, results: List[RetrievalResult]) -> pd.DataFrame:
        """
        Log retrieval results to Phoenix
    ...
    def run_phoenix_evals(
        ...
        documents_col: str = "retrieved_chunk"
    ):
        """
        Run Phoenix built-in evaluations
        
        Args:
            df: DataFrame with query and document columns
            queries_col: Column name for queries
            documents_col: Column name for documents
        """
...
def main():
    ...
    try:
        ...
        phoenix_tracer = PhoenixRAGTracer()  # Will use .phoenix/ directory
        ...
        # Log to Phoenix
        logger.info("Logging results to Phoenix...")
        results_df = phoenix_tracer.log_retrieval_results(results)
        print(f"\nResults logged to Phoenix. View at: {phoenix_tracer.session.url}")
        ...
        print("\n" + "="*60)
        print("Evaluation complete! Check Phoenix UI for detailed traces.")
        print("="*60)
